...
def _get_prowler_results(options, prowler_check):
  ...
  return _run_prowler('-b -c {check} -p {aws_profile} -M mono'.format(check=prowler_check,
...
def _send_prowler_results(prowler_results, _prowler_version, options):
  print("PROWLER RESULTS:")
  print(prowler_results)
  ...
  for _check_result in prowler_results.splitlines():
    ...
    try:
      ...
      _check_result2 = "\"" + _check_result + "\""
      DICT = {}
      _check_result = json.loads(TEMPLATE_CHECK.format(DICT))
      _check_result['result'] = _check_result2
      #_check_result = json.loads(TEMPLATE_CHECK.format(_check_result))
      print(_check_result)
      print(type(_check_result))
      print("SUCCESS!!!!!! DICT: ")
    except:
